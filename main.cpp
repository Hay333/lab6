...
template <typename T1, typename T2> class DoubleBox
{
    ...
    T2 data2;
public:
    DoubleBox(): data1(0), data2(0){};// конструктор по умолч.
    DoubleBox(const T1 value1, const T2 value2):data1(value1), data2(value2) {};// конструктор инициализации 
    T1 get1() const;
    T2 get2() const;
    void get() const;//метод get
    void set(const T1 value1, const T2 value2);//метод set
};
